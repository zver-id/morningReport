...
def create_chart_all_tickets(book):
    sheet = book["Графики"]
    last_column = book["tables"].max_column
    start_column = last_column - count_of_days_on_chart  #отображать данные будем за 70 рабочих дней

    chart = LineChart()
    chart.title = "Количество обращений в состоянии 'В работе'"
    ...
    min_row=32
    max_row=45
    series = Reference(book["tables"], min_col=start_column, max_col= last_column, min_row=min_row, max_row=max_row)
    chart.add_data(series, from_rows=True, titles_from_data=True)
...
def create_chart_older_two_weeks(book):
    ...
    series = Reference(book["tables"], min_col=start_column, max_col= last_column, min_row=54, max_row=54)
    chart.add_data(series, from_rows=True)
    series = Reference(book["tables"], min_col=start_column, max_col= last_column, min_row=63, max_row=63)
    chart.add_data(series, from_rows=True)
...
def create_chart_tail(book):
    sheet = book["Графики"]
    last_column = book["tables"].max_column - 1
    start_column = last_column - count_of_days_on_chart

    chart = LineChart()
    chart.title = "Общее количество не закрытых обращений"
    chart.anchor = anchor_cell_increment()
    chart.width = width_of_chars
    chart.height = height_of_chars
    ...
    series = Reference(book["tables"], min_col=start_column, max_col= last_column, min_row=68, max_row=68)
    chart.add_data(series, from_rows=True)
...
def create_chart_tail_older_4_weeks(book):
    sheet = book["Графики"]
    last_column = book["tables"].max_column - 1
    start_column = last_column - count_of_days_on_chart

    chart = LineChart()
    chart.title = "Количество не закрытых обращений старше 4 недель"
    chart.anchor = anchor_cell_increment()
    chart.width = width_of_chars
    chart.height = height_of_chars
    ...
    series = Reference(book["tables"], min_col=start_column, max_col= last_column, min_row=67, max_row=67)
    chart.add_data(series, from_rows=True)
